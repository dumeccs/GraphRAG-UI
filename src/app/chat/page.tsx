...
export default function Component() {
  ...
  const tempSources = useRef<string[]>([]);

  const FCAImage = "https://www.fca.org.uk/themes/custom/fca/images/logo.png";
  const CMAImage =
    "https://assets.publishing.service.gov.uk/media/59fa00e7e5274a5654e4e39a/CMA_Logo_01.png";
  ...
  const handleSubmit = useCallback(
    (e: any) => {
      ...
      const HOMEPAGE =
        "https://anti-greenwashing-graphrag-production.up.railway.app";
        // const HOMEPAGE = "http://localhost:8000";

      const es = new EventSource(`${HOMEPAGE}/chat?query=${input}`);
      es.addEventListener("message", function (event) {
        ...
        if (event.data == "END") {
          ...
          console.log(event.data.slice("<SOURCES>".length));
          tempSources.current = JSON.parse(
            event.data.slice("<SOURCES>".length)
          );
        } else {
          setNewMessage((nm) => {
    ...
    >
      ...
      >
        <div className="flex items-center mb-8">
          <a href="/" className="flex items-center space-x-2">
            ...
            <span className="font-bold text-xl text-white">Elohgrnwize</span>
          </a>
        </div>
      ...
      >
        ...
        >
          ...
          >
            <div
              className={`flex-1 flex bg-gray-50 rounded-2xl p-6 mb-4 min-h-full ${
                ...
                <div className="space-y-4 flex-1 w-full h-full">
                  {messages.map((message) => (
                    ...
                    >
                      ...
                      <div
                        className={`max-w-[70%] rounded-3xl p-3 py-2 mx-2 relative ${
                          message.role === "user"
                            ? "bg-[#08193b] text-white"
                            : "bg-gray-200 text-gray-800 py-6"
                        }`}
                      >
                        ...
                        {message.role == "assistant" && (
                          <Button
                            variant={"ghost"}
                            className="absolute right-4 bottom-4 px-2 py-1 hover:bg-greenwise-dark-blue hover:text-white"
                          >
                            <Copy size={16} />
                          </Button>
                        )}
        ...
        <div className="bg-white p-6 pt-0">
          ...
          >
            <div
              contentEditable
              ref={inputRef}
              className="min-h-[80px] max-h-[300px] w-full rounded-xl border  bg-transparent p-3 text-sm shadow-sm placeholder:text-muted-foreground focus:ring-2 focus:ring-offset-2 ring-[#08193b] outline-none disabled:cursor-not-allowed disabled:opacity-50 mt-4"
            ...
            <Button
              ...
              size="icon"
              className="bg-[#08193b] absolute text-white rounded-full bottom-2 right-2 flex items-center justify-center transition-all duration-300 shadow-lg hover:shadow-xl"
